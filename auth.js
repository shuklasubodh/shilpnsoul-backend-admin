...
export async function authenticate(req,res,next){try{const [scheme,token,extra]=String(req.get('authorization')||'').trim().split(/\s+/);if(scheme?.toLowerCase()!=='bearer'||!token||extra)throw 0;const claims=jwt.verify(token,secret(),{algorithms:['HS256'],issuer:'shilpnsoul-api',audience:'shilpnsoul-web'});if(!/^\d+$/.test(String(claims.sub)))throw 0;const rows=await sql`SELECT id,first_name,last_name,email,phone,role,is_active FROM users WHERE id=${claims.sub}`;if(!rows[0]?.is_active)throw 0;req.auth=claims;req.user=rows[0];next()}catch{return res.status(401).json({error:'Authentication required.'})}}
